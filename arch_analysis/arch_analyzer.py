...
class ArchitectureAnalyzer:
    ...
    def print_analysis_summary(self, analysis: Dict[str, Any]):
        ...
        print("\nKey Components:")
        components = [c for c in analysis.get('components', []) if c is not None]
        for component in sorted(components)[:10]:  # Show top 10
            print(f"- {component}")
            
        print("\nControl Flow Patterns:")
        control_patterns = defaultdict(int)
        for pattern in analysis.get('control_flow_patterns', []):
            if isinstance(pattern, dict) and 'node_type' in pattern:
        ...
        for pattern in analysis.get('data_flow_patterns', []):
        ...
        # Display top 5 components with their relationships
        for source, targets in sorted(relationships.items())[:5]:  # Show top 5
            valid_targets = [t for t in targets if t]
            if valid_targets:
                print(f"- {source} -> {', '.join(valid_targets)}")
